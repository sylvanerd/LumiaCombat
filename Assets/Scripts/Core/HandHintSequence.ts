import {
  HandAnimationClipInfo,
  HandAnimationsLibrary,
  HandMode,
  InteractionHintController
} from "Spectacles 3D Hand Hints.lspkg/Scripts/InteractionHintController"
...
export class HandHintSequence extends BaseScriptComponent {
  ...
  startHandGrabHint() {
    ...
    const itemA: HandAnimationClipInfo = new HandAnimationClipInfo(
      HandMode.Right,
      HandAnimationsLibrary.Right.PalmGrabX,
      this.handHintLocalPosition
    )
    sequence.push(itemA)
  ...
  private startHandPointHint() {
    ...
    const itemA: HandAnimationClipInfo = new HandAnimationClipInfo(
      HandMode.Right,
      HandAnimationsLibrary.Right.FingerSwipeX,
      this.handHintLocalPosition
    )
    sequence.push(itemA)
