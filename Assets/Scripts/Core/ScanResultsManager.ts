...
export class ScanResultsManager extends BaseScriptComponent {
  ...
  onScanResult(myScanResult: Bluetooth.ScanResult) {
    const nextScanResultSpot = this.getNextScanResultSpot()
    ...
    const myParent: SceneObject | null = nextScanResultSpot ? nextScanResultSpot : null
    const show: boolean = nextScanResultSpot ? true : false

    const newSo = this.scanResultPfb.instantiate(myParent)

    const screenTransform = newSo.getComponent("Component.ScreenTransform")
    screenTransform.offsets.setCenter(new vec2(0, 0))
    newSo.enabled = true

    const scanResult = newSo.getComponent("ScriptComponent") as ScanResult
    ...
    for (let i = 0; i < allScanResults.length; i++) {
      if (global.deviceInfoSystem.isEditor() || LensInitializer.getInstance().isNoBleDebug) {
        if (
          allScanResults[i]
            .getSceneObject()
            .uniqueIdentifier.includes(selectedScanResult.getSceneObject().uniqueIdentifier)
        ) {
