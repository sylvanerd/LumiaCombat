...
export class ButtonFeedback_ForceVisualState extends BaseScriptComponent {
  ...
  public onCodeChangeButtonState() {
    if (
      this.buttonFeedback.meshToggledIdleMaterial !== undefined &&
      this.buttonFeedback.meshIdleMaterial !== undefined
    ) {
      try {
        this.changeButtonState(
          this.toggleButton.isToggledOn
            ? this.buttonFeedback.meshToggledIdleMaterial
            : this.buttonFeedback.meshIdleMaterial
        )
      } catch (error) {
        reportError(error)
      }
